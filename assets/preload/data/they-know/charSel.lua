...
local song = 'tea-time'; --If you want to have a song play while people are in this menu. Remove this line entirely if you don't want one.
local displayNameX = -200; -- Offset for Display name
local displayNameY = 0; -- Offset for Display name
local customCameraPos = true
local camX = 600
local camY = 400
...
local characterList = { -- The list of characters
	...
	{
		opponent = 'spidey-magic',
		displayName = 'Magic Suit',
	},
	{
		opponent = 'spidey-hybrid',
		displayName = 'Hybrid Suit',
	},
	{
		opponent = 'spidey-unmasked',
		displayName = 'Unmasked Suit',
	}
}
...
function onStartCountdown()
	...
	if not hasSelectedCharacter and not isOnCharMenu then
		...
		setObjectCamera('displayname', 'camHUD');
		...
		setObjectCamera('rightarrow', 'camHUD');
		setObjectCamera('leftarrow', 'camHUD');
		...
		if(showSongBF) then table.insert(characterList,1,{
			name=origBF,
			displayName="Default Suit from Song"}) 
		end
		setProperty('inCutscene', true);
		setProperty('generatedMusic', false);
		setProperty('boyfriend.stunned', true);

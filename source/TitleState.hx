...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		FlxG.save.bind('spidey', 'teamDrift');
		...
		if (!transitioning && skippedIntro)
		{
			if(pressedEnter)
			{
				...
				new FlxTimer().start(1, function(tmr:FlxTimer)
				{
					...
					if(!FlxG.save.data.hasSeenMessage){

						// Message thingy
						FlxG.save.data.hasSeenMessage = true;
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				case 1:
					...
					#else
					createCoolText(['Driftyboyy0', 'Lil Tico', 'Shekkai47']);
					#end
				...
				case 4:
					...
					#else
					createCoolText(['ProwerPower', 'BigWes99', 'Magi Wagi', 'Spring']);
					#end
				// credTextShit.text += '\npresent...';
